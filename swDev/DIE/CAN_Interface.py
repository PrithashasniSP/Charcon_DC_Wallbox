...
received_message_vSECC = deque(maxlen=50)
received_message_cCON = deque(maxlen=50)
sendFrames_100ms = {
    0x186:None,
    0x182:None,
    0x188:None,
...
#List of Frames recieved from v.SECC and CharCon
VseccRcvFrmList = [0x301, 0x308, 0x30D, 0x30B, 0x30A, 0x306]
CConRcvFrmList = [0x5C1, 0x5C2, 0x5C0, 0x5C3, 0x5D0]
...
def Create_Frame(frame_id, matrix):
    ...
    for signal_name, signal_info in signals.items():
        ...
        try:            
            ...
            print(f"Error \"{e}\" Converting signal {signal_name}")
...
# Create a function to receive CharCon CAN messages
def receive_CCon_can_messages():
    global received_message_cCON
    
    # Define a list of CAN IDs to filter (adjust as needed)
    # can_ids_to_filter = [0x301, 0x308, 0x30D, 0x30B, 0x30A, 0x5C1, 0x5C2, 0x5C0, 0x5C3, 0x5D0]

    # Create a list of filters
    CConfilters = [{"can_id": canId, "can_mask": 0x7FF, "extended": False} for canId in CConRcvFrmList]

    # Create a CAN bus interface object (adjust the channel name as needed) and Set the filters for the CAN bus
    CCon_bus = can.interface.Bus(channel="can0", bustype="socketcan", can_filters=CConfilters)
    
    while True:
        try:
            received_message = CCon_bus.recv(timeout=0.1)  # Set a timeout (in seconds) for receiving CharCon messages 
            
            if(received_message is not None):
                received_message_cCON.append(received_message)
...
# Create a function to receive CharCon CAN messages
def receive_Vsecc_can_messages():
    global received_message_vSECC
    
    # Define a list of CAN IDs to filter (adjust as needed)
    # can_ids_to_filter = [0x301, 0x308, 0x30D, 0x30B, 0x30A, 0x5C1, 0x5C2, 0x5C0, 0x5C3, 0x5D0]

    # Create a list of filters
    Vseccfilters = [{"can_id": canId, "can_mask": 0x7FF, "extended": False} for canId in VseccRcvFrmList]

    # Create a CAN bus interface object (adjust the channel name as needed) and Set the filters for the CAN bus
    Vsecc_bus = can.interface.Bus(channel="can0", bustype="socketcan", can_filters=Vseccfilters)
    
    while True:
        try:
            received_message = Vsecc_bus.recv(timeout=0.25)  # Set a timeout (in seconds) for receiving V.SECC messages 
            
            if(received_message is not None):
                if(received_message.arbitration_id == 0x306):
                    Signals.StopChargingEv = True
                else:
                    received_message_vSECC.append(received_message)
            ...
            for data in sendFrames_100ms.values():
                ...
                if(data is not None):
                    try:
                        ...
                        # Handle the transmit buffer full error here, e.g., retry or log the error.
                        print("Transmit buffer full. Message not sent.")
...
def ProcessCAN():
    global received_message_vSECC, received_message_cCON, VseccRcvFrmList, CConRcvFrmList,sendFrames_100ms, sendFrames_250ms
    
    # Create a thread for receiving CharCon CAN messages
    recv_CCon_can_thread = threading.Thread(target=receive_CCon_can_messages)
    recv_CCon_can_thread.daemon = True  # Set the thread as a daemon so it will exit when the main program exits

    # Start the CAN thread
    recv_CCon_can_thread.start()
    
    # Create a thread for receiving CharCon CAN messages
    recv_Vsecc_can_thread = threading.Thread(target=receive_Vsecc_can_messages)
    recv_Vsecc_can_thread.daemon = True  # Set the thread as a daemon so it will exit when the main program exits

    # Start the CAN thread
    recv_Vsecc_can_thread.start()
    ...
    nextCConCheck = time.perf_counter()+0.1
    nextVseccCheck = time.perf_counter()+0.25

    # Main program
    try:
        while True:            
            if(time.perf_counter()>= nextCConCheck):
                Signals.CntrCharConFrameRecv += 1
                nextCConCheck = time.perf_counter()+0.1
            
            if(time.perf_counter()>= nextVseccCheck):
                Signals.CntrVseccFrameRecv += 1
                nextVseccCheck = time.perf_counter()+0.25
            
            if received_message_vSECC:                
                received_message_element = received_message_vSECC.popleft()
                Signals.CntrVseccFrameRecv = 0
                # Extract the CAN data from CAN frame
                extract_signals(received_message_element, CAN_Matrix.DataBase)
                    
            if received_message_cCON:
                received_message_element = received_message_cCON.popleft()
                Signals.CntrCharConFrameRecv = 0
                # Extract the CAN data from CAN frame
                extract_signals(received_message_element, CAN_Matrix.DataBase)       
            ...
            for frame in sendFrames_100ms.keys():
                ...
                if data is not None:
                    sendFrames_100ms[frame] = can.Message(arbitration_id=frame, dlc=8, data=data, is_extended_id=False)
