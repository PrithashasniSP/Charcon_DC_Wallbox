...
recv_read_queue = deque(maxlen=10)

#List of Frames recieved from v.SECC and CharCon
VseccRcvFrmList = [0x301, 0x308, 0x30D, 0x30B, 0x30A]
...
def receive_can_messages():
    global received_message,recv_read_queue
    ...
    while True:
        try:
            ...
            received_frame = bus.recv(timeout=0.25)  # Set a timeout (in seconds) for receiving messages
            
            if received_frame.arbitration_id not in recv_read_queue:
                recv_read_queue.append(received_frame.arbitration_id)
                received_message[received_frame.arbitration_id] = received_frame
...
def ProcessCAN():
    global received_message, VseccRcvFrmList, CConRcvFrmList,sendFrames_100ms, sendFrames_250ms
    
    # Create a thread for receiving CAN messages
    recv_can_thread = threading.Thread(target=receive_can_messages)
    recv_can_thread.daemon = True  # Set the thread as a daemon so it will exit when the main program exits

    # Start the CAN thread
    recv_can_thread.start()
    ...
    try:
        while True:            
            ...
            received_message_Copy = received_message.copy()
            
            for frameId, frameData in received_message_Copy.items():                    
                    # reset the vSECC or CharCon CAN counter
                    if frameId in VseccRcvFrmList:
                        Signals.CntrVseccFrameRecv = 0
                    elif frameId in CConRcvFrmList:
                        Signals.CntrCharConFrameRecv = 0
                    
                    # Extract the CAN data from CAN frame
                    extract_signals(frameData, CAN_Matrix.DataBase)

                    # Set received_message back to None to clear it for the next message
                    received_message.pop(frameId)
